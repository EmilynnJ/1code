...
export function initAnalytics() {
  ...
  if (!POSTHOG_DESKTOP_KEY) {
    console.log(
      "[Analytics] Skipping PostHog initialization (no key configured)",
    );
    return;
  }
...
export function trackAppOpened() {
  const hasLaunchedBefore = store.get("has_launched_before", false);
  const isFirstLaunch = !hasLaunchedBefore;

  if (isFirstLaunch) {
    store.set("has_launched_before", true);
  }
